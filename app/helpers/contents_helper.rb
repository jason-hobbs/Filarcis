module ContentsHelper
  ...
  def file_image(content)
    ...
    if content.public
      if image.include?('dmg') || image.include?('zip')  || image.include?('exe') || image.include?('default2')
        content_tag(:div, image_tag(image, :style => 'float:left;', :class => "img_preview", :id => content.id) + content_tag(:div, best_in_place(content, :name, :url => project_content_path(@project,content), :class => "input_field"), :class => "file_name"), :class => "fileopen_row") + content_tag(:div, "Share link:") + text_field(:share, :url, :value => "#{request.host}#{content.location.url}", :readonly => "readonly", :size => 50) + link_to("Download File", download_file_project_contents_path(:content => content.id), :class => 'button tiny radius space', :target => "_blank") + link_to("Unshare", share_project_contents_path(:content => content.id), :remote => true, :class => 'button tiny radius space')
      else
