module ContentsHelper
  ...
  def file_image(content)
    case content.content_type
      when /^image/
        if content.public
          content_tag(:div, image_tag(content.location.url(:thumb).to_s, :style => 'float:left;', :class => "img_preview", :id => content.id) + content_tag(:div, best_in_place(content, :name, :url => project_content_path(@project,content), :class => "input_field"), :class => "file_name"), :class => "fileopen_row") + text_field(:share, :url, :value => "#{request.host}#{content.location.url}", :readonly => "readonly") + link_to("Download File", download_file_project_contents_path(:content => content.id), :class => 'button tiny radius space', :target => "_blank") + link_to("View File", view_file_project_contents_path(:content => content.id), :class => 'button tiny radius space', :target => "_blank") + link_to("Unshare", share_project_contents_path(:content => content.id), :remote => true, :class => 'button tiny radius space')
        else
