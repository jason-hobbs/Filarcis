  ...
  def nested_folders(folders)
    folders.map do |folder, sub_folders|
      ...
      if folder.has_children?
        content_tag(:div, link_to(image_tag("/images/arrow-closed.png", {:size=>"15" }), "#", :class => "arrow") + link_to(image_tag("folder2.png", {:size=>"25" }), project_contents_path(folder), remote: true, :class => "highlight") + link_to("#{folder.name}<br>".html_safe, project_contents_path(folder), remote: true), :class => "folder-nav", :id => folder.id) + content_tag(:div, nested_folders(sub_folders), :class => "nested_folders")
      else
